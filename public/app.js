...
function hero(){
 ...
 return '<section class="hero autoPartsHero '+(first.image?'hasHeroBackground':'')+'">'+(slides.length?'<div class="homeHeroSlider" id="homeHeroSlider">'+slides.map((s,i)=>'<div class="homeHeroSlide '+(i===0?'active':'')+'" data-hero-black="'+esc(s.black||'')+'" data-hero-red="'+esc(s.red||'')+'" data-hero-description="'+esc(s.description||'')+'" data-hero-align="'+esc(s.align||'left')+'" style="background-image:url(\''+esc(s.image)+'\')"></div>').join('')+'</div>':'')+text(first)+'</section>';
}
...
function initHomeHeroSlider(images){
 ...
 if(list0.length)applyHomeHeroText(list0[0]);
 if(!images||images.length<2)return;
 let idx=0;
 window.__homeHeroSliderTimer=setInterval(()=>{const el=document.getElementById('homeHeroSlider');if(!el){clearInterval(window.__homeHeroSliderTimer);return}const slides=[...el.querySelectorAll('.homeHeroSlide')];if(!slides.length)return;slides[idx]?.classList.remove('active');idx=(idx+1)%slides.length;slides[idx]?.classList.add('active');applyHomeHeroText(slides[idx]);},4000);
}
...
function autoHeroSettingsMarkup(){
 ...
 const gallery=slides.length?'<div class="tyreAdminImageGallery">'+slides.map((s,i)=>'<button type="button" class="tyrePromoImageButton" onclick="toggleAutoHeroText('+i+')"><img src="'+esc(s.image||'')+'" alt="Hero image '+(i+1)+'"><span>IMAGE '+(i+1)+'</span><b>EDIT TEXT</b></button>').join('')+'</div>':'<div class="tyreAdminPreview emptyPreview">No images uploaded</div>';
 const editors=slides.map((s,i)=>'<div class="tyrePromoTextCard"><div id="autoHeroTextForm'+i+'" class="tyrePromoTextForm" hidden><div class="tyreAdminSubHead"><span>Image '+(i+1)+' text</span><small>Edit the text shown while this image is displayed.</small></div><div class="tyrePromoTextFields"><div class="formGroup"><label>Black headline</label><input id="autoHeroBlack'+i+'" class="input" value="'+esc(s.black||'')+'"></div><div class="formGroup"><label>Red headline</label><input id="autoHeroRed'+i+'" class="input" value="'+esc(s.red||'')+'"></div><div class="formGroup"><label>Description</label><textarea id="autoHeroDesc'+i+'" class="input" rows="2">'+esc(s.description||'')+'</textarea></div><div class="formGroup"><label>Text alignment</label><select id="autoHeroAlign'+i+'" class="select"><option value="left" '+(s.align==='left'?'selected':'')+'>Left</option><option value="center" '+(s.align==='center'?'selected':'')+'>Center</option><option value="right" '+(s.align==='right'?'selected':'')+'>Right</option></select><small class="helpText">Alignment applies to this hero image only.</small></div></div></div></div>').join('');
 return '<hr class="adminDivider"><h3>Homepage hero</h3><p class="muted">Each hero image has its own text and alignment. Text changes automatically with the image.</p><div class="formGroup"><label>Hero slideshow images</label><input id="heroFiles" type="file" accept="image/*" class="input" multiple onchange="previewAutoHeroUploads(this)"><small class="helpText">Select multiple images. Each image can have separate hero text and left, center or right alignment.</small><div id="autoHeroImageGallery">'+gallery+'</div></div><div id="autoHeroTextEditors" class="tyrePromoTextEditors">'+editors+'</div>';
}
function renderAutoHeroEditors(slides){
 ...
 const list=Array.isArray(slides)?slides:[];gallery.innerHTML=list.length?'<div class="tyreAdminImageGallery">'+list.map((s,i)=>'<button type="button" class="tyrePromoImageButton" onclick="toggleAutoHeroText('+i+')"><img src="'+esc(s.image||'')+'" alt="Hero image '+(i+1)+'"><span>IMAGE '+(i+1)+'</span><b>EDIT TEXT</b></button>').join('')+'</div>':'<div class="tyreAdminPreview emptyPreview">No images uploaded</div>';
 editor.innerHTML=list.map((s,i)=>'<div class="tyrePromoTextCard"><div id="autoHeroTextForm'+i+'" class="tyrePromoTextForm" hidden><div class="tyreAdminSubHead"><span>Image '+(i+1)+' text</span><small>Edit the text shown while this image is displayed.</small></div><div class="tyrePromoTextFields"><div class="formGroup"><label>Black headline</label><input id="autoHeroBlack'+i+'" class="input" value="'+esc(s.black||'')+'"></div><div class="formGroup"><label>Red headline</label><input id="autoHeroRed'+i+'" class="input" value="'+esc(s.red||'')+'"></div><div class="formGroup"><label>Description</label><textarea id="autoHeroDesc'+i+'" class="input" rows="2">'+esc(s.description||'')+'</textarea></div><div class="formGroup"><label>Text alignment</label><select id="autoHeroAlign'+i+'" class="select"><option value="left" '+(s.align==='left'?'selected':'')+'>Left</option><option value="center" '+(s.align==='center'?'selected':'')+'>Center</option><option value="right" '+(s.align==='right'?'selected':'')+'>Right</option></select><small class="helpText">Alignment applies to this hero image only.</small></div></div></div></div>').join('');
}
...
function previewAutoHeroUploads(input){
 const files=[...(input?.files||[])].filter(f=>f.type.startsWith('image/'));if(files.length!==((input?.files||[]).length))toast('Please select image files only');
 const saved=Array.isArray(db.settings?.heroSlides)?db.settings.heroSlides:[];const urls=files.map(f=>URL.createObjectURL(f));
 const all=[...saved,...urls.map(image=>({image,black:db.settings.heroBlack||'FIND THE RIGHT',red:db.settings.heroRed||'PARTS FOR YOUR CAR',description:db.settings.heroDescription||'High quality parts for all makes and models.',align:'left'}))];renderAutoHeroEditors(all);
}
