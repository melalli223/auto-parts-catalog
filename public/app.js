...
function tyreFinderShowResults(modelId){
 ...
 const ps=(db.tyres?.tyreProducts||[]).filter(p=>{
  const exactModel=String(p.modelId||p.vehicleModelId||p.vehicle_model_id||'')===String(m.id);if(!exactModel)return false;
  if(!modelTypeId)return true;
  ...
  return !productTypeId&&!productTypeTitle||productTypeId===modelTypeId||!!type&&productTypeTitle===String(type.title||'').trim().toLowerCase();
 });
