...
const promoSection=`<section class="tyreAdminCard tyreAdminPromoSection" id="tyrePromoSection"><div class="tyreCardHead"><div><span class="eyebrow">03 · PROMOTIONAL PANELS</span><h3>Winter, Summer & Custom Wheels</h3><p>Each panel has its own text, image and enquiry message.</p></div></div>
 ...
 const brandsSection=`<section class="tyreAdminCard" id="tyreBrandsSection"><div class="tyreCardHead"><div><span class="eyebrow">04 · BRANDS</span><h3>Tyre Brands</h3><p>Manage each tyre brand logo and its own page header image. The header image is separate for every brand.</p></div><button type="button" class="ghost" onclick="addTyreBrand()">+ ADD BRAND</button></div>
    <div class="tyreAdminBrandGrid">${brands.map((x,i)=>`<article class="tyreAdminBrandItem"><div class="tyreBrandNumber">BRAND ${i+1}<button type="button" class="tyreRemoveItemBtn" onclick="removeTyreBrand(${i})">Remove</button></div>${preview(x.image,(x.name||'Brand')+' logo')}${tyreInputRow('Brand name',`tb${i}n`,x.name)}<div class="formGroup"><label>Logo</label><input id="tb${i}i" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'tb${i}i')"></div><div class="tyreAdminSubSection"><div class="tyreAdminSubHead"><span>Brand page header image</span><small>This image belongs only to ${esc(x.name||'this brand')} and is used at the top of its tyre results page, like the Featured Tyre Types header.</small></div>${preview(x.headerImage,(x.name||'Brand')+' page header')}<div class="formGroup"><label>Header image</label><input id="tb${i}h" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'tb${i}h')"></div></div></article>`).join('')}</div>
   </section>`;
