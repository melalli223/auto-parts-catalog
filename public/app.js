...
function showTyreProductResults(title,filter,brandHeader=null){const ps=(db.tyres?.tyreProducts||[]).filter(p=>filter(p));setNav('');const header=brandHeader?'<div class="tyreBrandResultsHeader"><button class="tyreBrandBack" type="button" onclick="tyres()"><span>←</span> BACK TO TYRES</button><div class="tyreBrandHero"><div class="tyreBrandHeroImage"><img src="'+esc(brandHeader.image||placeholder(brandHeader.name))+'" alt="'+esc(brandHeader.name)+'"></div><div class="tyreBrandHeroCopy"><span class="eyebrow">TYRE BRAND</span><h1>'+esc(brandHeader.name)+'</h1><p>Browse available '+esc(brandHeader.name)+' tyres.</p><strong>'+ps.length+' PRODUCT'+(ps.length===1?'':'S')+'</strong></div></div></div>':'<button class="tyrePlaceholderBack" onclick="tyres()">← Back to Tyres</button>';render('<section class="tyreExactPage"><div class="tyreDesktop"><div class="tyreProductResultsPage">'+header+(brandHeader?'':'<div class="tyreProductResultsHead"><span>TYRE CATALOGUE</span><h1>'+esc(title)+'</h1><p>'+ps.length+' tyre product'+(ps.length===1?'':'s')+' found.</p></div>')+'<div class="tyrePublicProductGrid">'+(ps.map(tyreProductCard).join('')||'<div class="empty">No tyre products found for this selection.</div>')+'</div></div></div></section>');ensureTyreCompareUI();}
