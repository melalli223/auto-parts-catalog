...
function normalizeDb(){
 db.settings={...clone(defaults.settings),...(db.settings||{})}; db.tyres={
  ...
  hero:{...clone(defaultTyres.hero),...((db.tyres||{}).hero||{})},findByCar:{...clone(defaultTyres.findByCar),...((db.tyres||{}).findByCar||{})},findByNumber:{...clone(defaultTyres.findByNumber),...((db.tyres||{}).findByNumber||{})},promoImages:Array.isArray(db.tyres?.promoImages)?db.tyres.promoImages.filter(Boolean):[],promoTexts:Array.isArray(db.tyres?.promoTexts)?db.tyres.promoTexts:clone(defaultTyres.promoTexts),features:Array.isArray(db.tyres?.features)&&db.tyres.features.length===3?db.tyres.features:clone(defaultTyres.features),brands:Array.isArray(db.tyres?.brands)&&db.tyres.brands.length>0?db.tyres.brands:clone(defaultTyres.brands),featured:Array.isArray(db.tyres?.featured)&&db.tyres.featured.length>0?db.tyres.featured:clone(defaultTyres.featured)};
...
function initTyresPromoSlideshow(){
 ...
 const defaults=clone(defaultTyres.promoTexts||[]);
 const promoTexts=images.map((_,i)=>({...defaults[i%defaults.length],...((db.tyres?.promoTexts||[])[i]||{})}));
 ...
 const applyPromoText=(i)=>{
  ...
  if(desc)desc.innerHTML=String(t.desc||'').replace(/\n/g,'<br>');
  if(button)button.textContent=t.button||'EXPLORE TYRES →';
 };
 applyPromoText(0);
...
const promoSection=`<section class="tyreAdminCard tyreAdminPromoSection" id="tyrePromoSection"><div class="tyreCardHead"><div><span class="eyebrow">03 · PROMOTIONAL PANELS</span><h3>Winter, Summer & Custom Wheels</h3><p>Each panel has its own text, image and enquiry message.</p></div></div>
 ...
 const promoTextEditor=promoTextItems.length?'<div class="tyrePromoTextEditors">'+promoTextItems.map((x,i)=>'<div class="tyreAdminSubSection tyrePromoTextCard"><div class="tyreAdminSubHead"><span>Image '+(i+1)+' text</span><small>Text shown while this image is displayed.</small></div><div class="tyrePromoTextFields"><div class="formGroup"><label>Heading</label><input id="tPromoTitle'+i+'" class="input" value="'+esc(x.title||'')+'"></div><div class="formGroup"><label>Highlight</label><input id="tPromoHighlight'+i+'" class="input" value="'+esc(x.highlight||'')+'"></div><div class="formGroup"><label>Description</label><textarea id="tPromoDesc'+i+'" class="input" rows="2">'+esc(String(x.desc||'').replace(/<br>/g,'\n'))+'</textarea></div><div class="formGroup"><label>Button text</label><input id="tPromoButton'+i+'" class="input" value="'+esc(x.button||'')+'"></div></div></div>').join('')+'</div>':'<div class="tyreAdminPreview emptyPreview">Upload images first to add their text.</div>';
 const shopTyresPromoSection=`<section class="tyreAdminCard tyreAdminPromoSection" id="tyreShopTyresPromoSection"><div class="tyreCardHead"><div><span class="eyebrow">SHOP TYRES PROMO</span><h3>Shop Tyres Promo Box</h3><p>Upload multiple background images for the Shop Tyres box on the Auto Parts homepage.</p></div></div><div class="tyreAdminSubSection"><div class="tyreAdminSubHead"><span>Promo slideshow images</span><small>Multiple images rotate automatically inside the full promo box.</small></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Promo images</label><input id="tPromoImages" type="file" accept="image/*" multiple class="input"><small class="helpText">Select multiple images at once. New uploads are added to the slideshow.</small>${imageGallery(t.promoImages,'Shop Tyres promo image')}</div><div></div></div></div>${promoTextEditor}</section>`;
