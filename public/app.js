...
function hero(){
 const images=Array.isArray(db.settings.heroImages)&&db.settings.heroImages.length
  ? db.settings.heroImages
  : (db.settings.heroImage?[db.settings.heroImage]:[]);
 const first=images[0]||'';
 return `<section class="hero autoPartsHero ${first?'hasHeroBackground':''}" ${first?`style="background-image:url('${esc(first)}')"`:''}>
   ${images.length>1?`<div class="homeHeroSlider" id="homeHeroSlider">${images.map((src,i)=>`<div class="homeHeroSlide ${i===0?'active':''}" style="background-image:url('${esc(src)}')"></div>`).join('')}</div>`:''}
   <div class="heroCopy">
...
function initHomeHeroSlider(images){
 ...
 window.__homeHeroSliderTimer=setInterval(()=>{
   const el=document.getElementById('homeHeroSlider');
   if(!el){
     clearInterval(window.__homeHeroSliderTimer);
     return;
   }
   const slides=el.querySelectorAll('.homeHeroSlide');
...
<span class="tyreFinderCardImage"><img src="${esc(findByCar.image||placeholder('Find Tyre by Car'))}" alt=""></span>
...
<span class="tyreFinderCardImage"><img src="${esc(findByNumber.image||placeholder('Find Tyre by Number'))}" alt=""></span>
...
function tyresAdmin(c){
 const t=db.tyres||clone(defaultTyres),contact=t.contact||defaultTyres.contact,hero=t.hero||defaultTyres.hero,features=t.features||defaultTyres.features,brands=t.brands||defaultTyres.brands,featured=t.featured||defaultTyres.featured,findByCar=t.findByCar||defaultTyres.findByCar,findByNumber=t.findByNumber||defaultTyres.findByNumber,heroImages=(hero.images&&hero.images.length?hero.images:[hero.image||defaultTyres.hero.image]);
 ...
 const heroSection=`<section class="tyreAdminCard tyreAdminHeroCard" id="tyreHeroSection"><div class="tyreCardHead"><div><span class="eyebrow">01 · HERO</span><h3>Hero section</h3><p>Control the main heading, description and sliding background images.</p></div></div>
    <div class="tyreAdminHeroLayout"><div>${tyreInputRow('White heading','thTitle',hero.title)}${tyreInputRow('Red heading','thRed',hero.red)}<div class="formGroup"><label>Description</label><textarea id="thDesc" class="textarea" rows="4">${esc(hero.description||'')}</textarea></div></div><div class="formGroup"><label>Hero image</label><label class="primary fileBtn tyreHeroUploadBtn">UPLOAD HOMEPAGE HERO IMAGES<input id="thAddImage" type="file" accept="image/*" multiple hidden onchange="addTyreHeroImage(this)"></label><small class="helpText">Select one or multiple images. They rotate automatically on the Tyre homepage.</small></div></div></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Bottom image</label><label class="primary fileBtn tyreBottomUploadBtn">UPLOAD HOMEPAGE BOTTOM IMAGE<input id="thBottomImg" type="file" accept="image/*" hidden onchange="prepareImageSelection(event,'thBottomImg')"></label><small class="helpText">This image appears at the bottom of the Tyre homepage.</small></div><div>${preview(t.bottomImage,'Tyre homepage bottom image')}</div></div><div class="tyreAdminFindSection"><div class="tyreCardHead"><div><span class="eyebrow">03 · FIND SECTION</span><h3>Find section images</h3><p>These images replace the icons beside the two tyre-finder options on the Tyre homepage.</p></div></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Find Tyre by Car image</label><label class="primary fileBtn tyreFindUploadBtn">UPLOAD FIND BY CAR IMAGE<input id="tbCarHeroImg" type="file" accept="image/*" hidden onchange="prepareImageSelection(event,'tbCarHeroImg')"></label><small class="helpText">Image shown beside “Find Tyre by Car”.</small></div><div>${preview(findByCar.image,'Find Tyre by Car image')}</div></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Find Tyre by Number image</label><label class="primary fileBtn tyreFindUploadBtn">UPLOAD FIND BY NUMBER IMAGE<input id="tbNumberHeroImg" type="file" accept="image/*" hidden onchange="prepareImageSelection(event,'tbNumberHeroImg')"></label><small class="helpText">Image shown beside “Find Tyre by Number”.</small></div><div>${preview(findByNumber.image,'Find Tyre by Number image')}</div></div></div>
   </section>`;
...
async function saveTyrePage(){try{const t=clone(db.tyres||defaultTyres);const q=id=>document.querySelector('#'+id);if(q('tbCarHeroImg')||q('tbCarImg')){t.findByCar=t.findByCar||{};const f=editedImage('tbCarHeroImg');if(f)t.findByCar.image=await uploadImage(f,'product-images','find-by-car-hero');const b=editedImage('tbCarImg');if(b)t.findByCar.bottomImage=await uploadImage(b,'product-images','find-by-car-bottom');}if(q('tbNumberHeroImg')||q('tbNumberImg')){t.findByNumber=t.findByNumber||{};const f=editedImage('tbNumberHeroImg');if(f)t.findByNumber.image=await uploadImage(f,'product-images','find-by-number-hero');const b=editedImage('tbNumberImg');if(b)t.findByNumber.bottomImage=await uploadImage(b,'product-images','find-by-number-bottom');}if(q('thTitle')){t.hero.title=q('thTitle').value.trim();t.hero.red=q('thRed').value.trim();t.hero.description=q('thDesc').value.trim();const hf=editedImage('thBottomImg');if(hf)t.bottomImage=await uploadImage(hf,'product-images','tyre-home-bottom');
...
async function saveSettings(){try{
 ...
 const heroImages=[...keptHeroImages];
 for(const file of heroFiles){
   if(!file.type.startsWith('image/'))throw new Error('Please select image files only');
   heroImages.push(await uploadImage(file,'hero','settings'));
 }

 const heroUrl=heroImages[0]||null;
 ...
 const payload={
  ...
  hero_black:document.querySelector('#hblack').value.trim(),
  hero_red:document.querySelector('#hred').value.trim(),
  hero_description:document.querySelector('#hdesc').value.trim(),
  hero_image_url:heroImages.length?JSON.stringify(heroImages):null,
 ...
 try{
   ...
   parsedHeroImages=Array.isArray(a)?a.filter(Boolean):(hv?[hv]:[]);
 }catch{
   parsedHeroImages=hv?[hv]:[];
 }
