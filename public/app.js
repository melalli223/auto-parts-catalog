...
function normalizeDb(){
 db.settings={...clone(defaults.settings),...(db.settings||{})}; db.tyres={
  ...
  hero:{...clone(defaultTyres.hero),...((db.tyres||{}).hero||{})},features:Array.isArray(db.tyres?.features)&&db.tyres.features.length===3?db.tyres.features:clone(defaultTyres.features),brands:Array.isArray(db.tyres?.brands)&&db.tyres.brands.length>0?db.tyres.brands:clone(defaultTyres.brands),featured:Array.isArray(db.tyres?.featured)&&db.tyres.featured.length>0?db.tyres.featured:clone(defaultTyres.featured)};
...
function hero(){
 ...
 return `<section class="hero autoPartsHero ${bg?'hasHeroBackground':''}" ${bg?`style="background-image:url('${esc(bg)}')"`:''}>
   <div class="heroCopy">
...
function tyresAdmin(c){
 const t=db.tyres||clone(defaultTyres),contact=t.contact||defaultTyres.contact,hero=t.hero||defaultTyres.hero,features=t.features||defaultTyres.features,brands=t.brands||defaultTyres.brands,featured=t.featured||defaultTyres.featured,heroImages=(hero.images&&hero.images.length?hero.images:[hero.image||defaultTyres.hero.image]);
...
const promoSection=`<section class="tyreAdminCard"><div class="tyreCardHead"><div><span class="eyebrow">02 · PROMOTIONAL PANELS</span><h3>Winter, Summer & Custom Wheels</h3><p>Each panel has its own text, image and enquiry message.</p></div></div>
 ...
 if(tyreAdminSubTab==='settings'){sectionsHtml=heroSection+contactSettingsSection+promoSection+bottomSection;showSaveBar=true}
...
async function saveTyrePage(){try{const t=clone(db.tyres||defaultTyres);const q=id=>document.querySelector('#'+id);if(q('thTitle')){t.hero.title=q('thTitle').value.trim();t.hero.red=q('thRed').value.trim();t.hero.description=q('thDesc').value.trim();
...
async function saveSettings(){try{const lf=editedImage('logoFile'),hf=editedImage('heroFile'),pf=editedImage('promoBackgroundFile');let logoUrl=db.settings.logo||null,heroUrl=db.settings.heroImage||null,promoBackgroundUrl=db.settings.promoBackground||null;if(lf)logoUrl=await uploadImage(lf,'logos','settings');if(hf)heroUrl=await uploadImage(hf,'hero','settings');if(pf)promoBackgroundUrl=await uploadImage(pf,'promo-background','settings');const payload={id:true,business_name:document.querySelector('#sname').value.trim(),tagline:document.querySelector('#stag').value.trim(),logo_url:logoUrl,phone:document.querySelector('#sphone').value.trim(),phone2:document.querySelector('#sphone2').value.trim(),phone3:document.querySelector('#sphone3').value.trim(),instagram_url:document.querySelector('#sinstagram').value.trim(),telegram_url:document.querySelector('#stelegram').value.trim(),facebook_url:document.querySelector('#sfacebook').value.trim(),tiktok_url:document.querySelector('#stiktok').value.trim(),youtube_url:document.querySelector('#syoutube').value.trim(),x_url:document.querySelector('#sx').value.trim(),about_title:document.querySelector('#aboutTitle').value.trim(),about_content:document.querySelector('#aboutText').value.trim(),whatsapp:document.querySelector('#swa').value.replace(/\D/g,''),email:document.querySelector('#semail').value.trim(),address:document.querySelector('#saddr').value.trim(),hero_black:document.querySelector('#hblack').value.trim(),hero_red:document.querySelector('#hred').value.trim(),hero_description:document.querySelector('#hdesc').value.trim(),
      ...
      promoBackground:data.promo_background_url||''};cacheDb();toast('Settings saved');setTimeout(()=>adminPanel('settings'),400)}catch(e){console.error(e);toast(e.message||'Could not save settings')}}
