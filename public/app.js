...
function normalizeDb(){
 ...
 db.tyres={
  ...
  hero:{...clone(defaultTyres.hero),...((db.tyres||{}).hero||{})},findByCar:{...clone(defaultTyres.findByCar),...((db.tyres||{}).findByCar||{})},findByNumber:{...clone(defaultTyres.findByNumber),...((db.tyres||{}).findByNumber||{})},promoImages:Array.isArray(db.tyres?.promoImages)?db.tyres.promoImages.filter(Boolean):[],promoTexts:Array.isArray(db.tyres?.promoTexts)?db.tyres.promoTexts:clone(defaultTyres.promoTexts),features:Array.isArray(db.tyres?.features)&&db.tyres.features.length===3?db.tyres.features:clone(defaultTyres.features),brands:Array.isArray(db.tyres?.brands)&&db.tyres.brands.length>0?db.tyres.brands:clone(defaultTyres.brands),featured:Array.isArray(db.tyres?.featured)&&db.tyres.featured.length>0?db.tyres.featured:clone(defaultTyres.featured),sizes:Array.isArray(db.tyres?.sizes)?db.tyres.sizes:[]};
  ...
  document.querySelector('#app').innerHTML=`
   ...
   <div class="tyreAdminShell ${adminSidebarOpen?'':'sidebarHidden'}">
    <aside class="tyreAdminSide">
     ...
     <!-- Tyre Sizes must remain directly below Featured Types in the sidebar. -->
