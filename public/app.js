...
function tyreBrandPage(idx){const t=db.tyres||defaultTyres,b=(t.brands||[])[idx];if(!b)return tyres();location.hash=`tyres/brand/${idx}`;showTyreProductResults(`${b.name} Tyres`,p=>String(p.brandId)===String(b.id)||String(p.brandId)===String(idx)||String(p.brand||'').toLowerCase()===String(b.name).toLowerCase(),b)}
