...
async function loadTyreDashboardEnquiries(range='month',customStart='',customEnd=''){
 ...
 try{
  ...
  const {data,error}=await supabaseClient.from('product_enquiries').select('*').order('created_at',{ascending:false}).limit(1000);if(error)throw error;
  const all=data||[],rows=all.filter(x=>{const brand=String(x.brand_name||'').trim().toLowerCase(),type=String(x.category_name||'').trim().toLowerCase(),size=String(x.model_name||'').trim().toLowerCase(),name=String(x.product_name||'').toLowerCase();return tyreBrands.includes(brand)||tyreTypes.includes(type)||tyreSizes.includes(size)||/\btyres?\b|\btires?\b/.test(name)});
  const now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate()),week=new Date(today);week.setDate(week.getDate()-week.getDay()),month=new Date(now.getFullYear(),now.getMonth(),1);
  let start,end=new Date(now);
  if(range==='today')start=today;
  else if(range==='week')start=week;
  else if(range==='month')start=month;
  else if(range==='custom'&&customStart){start=new Date(customStart+'T00:00:00');if(customEnd)end=new Date(customEnd+'T23:59:59.999')}
  else start=month;
  const inSelected=v=>{const x=new Date(v);return !Number.isNaN(x.getTime())&&x>=start&&x<=end};
  const selected=rows.filter(x=>inSelected(x.created_at));
  ...
  const days=Math.min(31,Math.max(1,Math.ceil((end-start)/86400000)+1)),daily=[];for(let i=days-1;i>=0;i--){const d=new Date(end);d.setHours(0,0,0,0);d.setDate(d.getDate()-i);const key=d.toISOString().slice(0,10);daily.push({date:key,count:selected.filter(x=>String(x.created_at||'').slice(0,10)===key).length})}
  ...
  return {total:selected.length,today:rows.filter(x=>inRange(x.created_at,today)).length,week:rows.filter(x=>inRange(x.created_at,week)).length,month:rows.filter(x=>inRange(x.created_at,month)).length,products:counts('product_name'),brands:counts('brand_name'),types:counts('category_name'),sizes:counts('model_name'),daily,statuses:statusCounts,recent:selected.slice(0,6),error:null,range,start:start.toISOString().slice(0,10),end:end.toISOString().slice(0,10)};
