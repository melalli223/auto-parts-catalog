...
function brands(ev=false){setNav('brands');location.hash='brands';const list=ev?db.brands.filter(b=>b.isEv===true):db.brands.filter(b=>b.isRegular!==false);render(`<div class="breadcrumb">Home <span>›</span> ${ev?'EV Brands':'Brands'}</div><div class="sectionHead"><div><span class="eyebrow">STEP 1</span><h2>SHOP BY <span class="accent">${ev?'EV BRAND':'CAR BRAND'}</span></h2></div></div><div class="grid brandGrid">${list.map(b=>brandCard(b,ev)).join('')||'<div class="empty">No brands added yet.</div>'}</div>`)}
