...
const promoSection=`<section class="tyreAdminCard tyreAdminPromoSection" id="tyrePromoSection"><div class="tyreCardHead"><div><span class="eyebrow">03 · PROMOTIONAL PANELS</span><h3>Winter, Summer & Custom Wheels</h3><p>Each panel has its own text, image and enquiry message.</p></div></div>
 ...
 const tyreProductList=Array.isArray(t.tyreProducts)?t.tyreProducts:[];
 const tyreWithImages=tyreProductList.filter(p=>p.image).length;
 const tyreMissingImages=tyreProductList.length-tyreWithImages;
 const tyreCoverage=tyreProductList.length?Math.round(tyreWithImages/tyreProductList.length*100):0;
 const tyreAvailable=tyreProductList.filter(p=>String(p.availability||'').toLowerCase()==='available').length;
 const tyreOut=tyreProductList.filter(p=>String(p.availability||'').toLowerCase()==='out of stock').length;
 const recentTyres=[...tyreProductList].slice(-6).reverse();
 const dashboardBody=`<div class="adminWelcome"><div><span class="eyebrow">COMMAND CENTER</span><h2>Tyre catalogue overview</h2><p class="muted">Manage tyre content, catalogue structure, and product readiness from one dashboard.</p></div><div class="adminDashboardActions"><button class="ghost" onclick="tyreAdminGo('products')">MANAGE PRODUCTS</button><button class="ghost" onclick="tyreAdminGo('settings')">TYRE SETTINGS</button><button class="primary" onclick="openTyreProductModal()">+ ADD TYRE PRODUCT</button></div></div>
 <div class="dashboardKpis"><div class="card dashKpi"><span>TYRE PRODUCTS</span><b>${tyreProductList.length}</b><small>${brands.length} brands · ${featured.length} types</small></div><div class="card dashKpi"><span>AVAILABLE</span><b>${tyreAvailable}</b><small>${tyreProductList.length?Math.round(tyreAvailable/Math.max(1,tyreProductList.length)*100):0}% of tyre products</small></div><div class="card dashKpi"><span>TYRE SIZES</span><b>${sizes.length}</b><small>Reusable sizes in the catalogue</small></div><div class="card dashKpi"><span>IMAGE COVERAGE</span><b>${tyreCoverage}%</b><small>${tyreMissingImages?tyreMissingImages+' missing':'All products have images'}</small></div></div>
 <div class="dashboardGrid"><section class="card dashboardPanel"><div class="dashboardPanelHead"><div><span class="eyebrow">TYRE CATALOG HEALTH</span><h3>Content readiness</h3></div><button class="ghost" onclick="tyreAdminGo('products')">VIEW PRODUCTS</button></div><div class="healthRows"><div><span>Tyre Brands</span><b>${brands.length}</b><i style="width:${brands.length?100:0}%"></i></div><div><span>Tyre Types</span><b>${featured.length}</b><i style="width:${featured.length?100:0}%"></i></div><div><span>Tyre Sizes</span><b>${sizes.length}</b><i style="width:${sizes.length?100:0}%"></i></div><div><span>Products with images</span><b>${tyreWithImages}/${tyreProductList.length}</b><i style="width:${tyreCoverage}%"></i></div></div><div class="healthAlerts">${tyreMissingImages?`<div class="healthAlert warn"><strong>${tyreMissingImages} tyre product${tyreMissingImages===1?'':'s'} missing images</strong><button class="ghost" onclick="tyreAdminGo('products')">REVIEW</button></div>`:''}${tyreOut?`<div class="healthAlert"><strong>${tyreOut} tyre product${tyreOut===1?'':'s'} marked out of stock</strong><span>Hidden from customers.</span></div>`:''}${!tyreMissingImages&&!tyreOut?'<div class="healthAlert good"><strong>Tyre catalogue looks healthy</strong><span>No obvious content warnings.</span></div>':''}</div></section>
 <section class="card dashboardPanel"><div class="dashboardPanelHead"><div><span class="eyebrow">RECENT TYRE PRODUCTS</span><h3>Latest tyre additions</h3></div><button class="ghost" onclick="openTyreProductModal()">+ PRODUCT</button></div>${recentTyres.length?`<div class="recentProducts">${recentTyres.map(p=>`<div class="recentProduct"><img src="${p.image||placeholder(p.name||'Tyre')}"><div><strong>${esc(tyreProductName(p)||p.name||'Tyre product')}</strong><small>${esc([p.tubeType,p.availability].filter(Boolean).join(' · '))}</small></div><span class="availability ${String(p.availability||'Available').toLowerCase().replace(/\\s+/g,'-')}">${esc(p.availability||'Available')}</span></div>`).join('')}</div>`:'<p class="muted">No tyre products yet. Add your first tyre product to start the tyre catalogue.</p>'}</section></div>
 <div class="quickActions"><button onclick="tyreAdminGo('brands')"><strong>+ Brand</strong><span>Add or arrange tyre brands</span></button><button onclick="tyreAdminGo('featured')"><strong>+ Tyre Type</strong><span>Manage tyre type cards</span></button><button onclick="tyreAdminGo('sizes')"><strong>+ Tyre Size</strong><span>Manage reusable tyre sizes</span></button><button onclick="openTyreProductModal()"><strong>+ Product</strong><span>Add a tyre product</span></button></div>`;
 ...
 const pageHeader=isDashboard?`
  <div class="adminWelcome">
   <div><span class="eyebrow">TYRE MANAGEMENT</span><h2>Tyre catalogue dashboard</h2><p class="muted">Manage the dedicated tyre catalogue separately from the main auto-parts catalog.</p></div>
   <div class="adminDashboardActions"><a class="ghost" href="/admin">← MAIN ADMIN</a><button class="viewSite" onclick="location.href='/#tyres'">VIEW TYRE PAGE</button></div>
  </div>`:``;
