...
function showTyreProductResults(title,filter,brandHeader=null){const ps=(db.tyres?.tyreProducts||[]).filter(p=>filter(p));setNav('');const header=brandHeader?(brandHeader.kind==='type'?'<div class="tyreTypeResultsHeader"><div class="tyreTypeHeaderCopy"><span class="eyebrow">FEATURED TYRE TYPE</span><h1>'+esc(String(brandHeader.name||'').toUpperCase())+' TYRES</h1><p>'+esc(brandHeader.description||('Browse available '+String(brandHeader.name||'').toUpperCase()+' tyres.'))+'</p></div></div>':'<div class="tyreBrandResultsHeader"><div class="tyreBrandHero"><img class="tyreBrandHeroImage" src="'+esc(brandHeader.image||placeholder(brandHeader.name))+'" alt="'+esc(brandHeader.name)+'"></div><div class="tyreBrandHeroCopy"><h1>'+esc(String(brandHeader.name||'').toUpperCase())+'<sup>®</sup></h1><p>'+esc(brandHeader.description||('Browse available '+String(brandHeader.name||'').toUpperCase()+' tyres.'))+'</p></div></div>'):'<button class="tyrePlaceholderBack" onclick="tyres()">← Back to Tyres</button>';render('<section class="tyreExactPage"><div class="tyreDesktop"><div class="tyreProductResultsPage">'+header+(brandHeader?'':'<div class="tyreProductResultsHead"><span>TYRE CATALOGUE</span><h1>'+esc(title)+'</h1><p>'+ps.length+' tyre product'+(ps.length===1?'':'s')+' found.</p></div>')+'<div class="tyreProductSlider"><button class="tyreProductSliderArrow tyreProductSliderPrev" type="button" aria-label="Previous products" onclick="tyreProductSliderScroll(-1)">‹</button><div class="tyrePublicProductGrid">'+(ps.map(tyreProductCard).join('')||'<div class="empty">No tyre products found for this selection.</div>')+'</div><button class="tyreProductSliderArrow tyreProductSliderNext" type="button" aria-label="Next products" onclick="tyreProductSliderScroll(1)">›</button></div><button class="tyrePlaceholderBack tyreResultsBottomBack" onclick="tyres()">← Back to Tyres</button></div></div></section>');ensureTyreCompareUI();}
