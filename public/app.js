...
function tyreProductCard(p){const name=tyreProductName(p),availability=p.availability||'Available',cls=String(availability).toLowerCase().includes('out')?'out':'';return '<article class="tyrePublicProductCard" onclick="tyreProductDetails(\\''+esc(p.id)+'\\')"><div class="tyrePublicProductImage">'+(p.image?'<img src="'+esc(p.image)+'" alt="'+esc(name)+'">':'<span>TYRE</span>')+'</div><div class="tyrePublicProductBody"><h3>'+esc(name)+'</h3><div class="tyrePublicProductSpecs"><span>Size <b>'+esc(size)+'</b></span><span>Type <b>'+esc(type)+'</b></span><span>Season <b>'+esc(p.season||'')+'</b></span><span>Tube <b>'+esc(p.tubeType||'')+'</b></span><span>Load/Speed <b>'+esc([p.loadIndex,p.speedRating].filter(Boolean).join('')||'—')+'</b></span></div><div class="tyrePublicProductBottom"><span class="tyreProductAvailability '+cls+'">'+esc(availability)+'</span><b>View Details →</b></div></div></article>'}
