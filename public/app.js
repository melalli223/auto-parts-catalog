...
function tyresAdmin(c){
 ...
 const heroSection=`<section class="tyreAdminCard tyreAdminHeroCard" id="tyreHeroSection"><div class="tyreCardHead"><div><span class="eyebrow">01 · HERO</span><h3>Hero section</h3><p>Control the main heading, description and sliding background images.</p></div></div>
    <div class="tyreAdminHeroLayout"><div>${tyreInputRow('White heading','thTitle',hero.title)}${tyreInputRow('Red heading','thRed',hero.red)}<div class="formGroup"><label>Description</label><textarea id="thDesc" class="textarea" rows="4">${esc(hero.description||'')}</textarea></div></div><div class="tyreHeroGalleryWrap"><label>Hero images (slideshow)</label><div class="tyreHeroGallery">${heroImages.map((src,i)=>`<div class="tyreHeroGalleryItem"><img src="${esc(src)}" alt="Hero ${i+1}">${heroImages.length>1?`<button type="button" class="tyreHeroRemoveBtn" onclick="removeTyreHeroImage(${i})" title="Remove">×</button>`:''}</div>`).join('')}</div><input id="thAddImage" type="file" accept="image/*" multiple class="input" onchange="addTyreHeroImage(this)"><small class="helpText">Upload one or multiple images. They rotate automatically on the public page.</small></div></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Bottom image</label><input id="thBottomImg" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'thBottomImg')"><small class="helpText">This image appears at the bottom of the Tyre homepage.</small></div><div>${preview(t.bottomImage,'Tyre homepage bottom image')}</div></div>
   </section>`;
