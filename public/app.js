...
async function loadTyreDashboardEnquiries(){
 const empty={total:0,today:0,week:0,month:0,products:[],brands:[],types:[],sizes:[],daily:[],statuses:[],recent:[],error:null};
 try{
  ...
  const {data,error}=await supabaseClient.from('product_enquiries').select('*').order('created_at',{ascending:false}).limit(500);
  ...
  const inRange=(v,d)=>{const x=new Date(v);return !Number.isNaN(x.getTime())&&x>=d};
  const counts=key=>{const m=new Map();rows.forEach(x=>{const k=String(x[key]||'Unknown').trim()||'Unknown';m.set(k,(m.get(k)||0)+1)});return [...m.entries()].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name)).slice(0,10)};
  const daily=[];for(let i=13;i>=0;i--){const d=new Date(day);d.setDate(d.getDate()-i);const key=d.toISOString().slice(0,10);daily.push({date:key,count:rows.filter(x=>String(x.created_at||'').slice(0,10)===key).length})}
  const statusNames=['New','Contacted','Quoted','Sold','Closed'],statusCounts=Object.fromEntries(statusNames.map(x=>[x,0]));rows.forEach(x=>{const s=statusNames.includes(x.status||'New')?(x.status||'New'):'New';statusCounts[s]++});
  return {total:rows.length,today:rows.filter(x=>inRange(x.created_at,day)).length,week:rows.filter(x=>inRange(x.created_at,week)).length,month:rows.filter(x=>inRange(x.created_at,month)).length,products:counts('product_name'),brands:counts('brand_name'),types:counts('category_name'),sizes:counts('model_name'),daily,statuses:statusCounts,recent:rows.slice(0,6),error:null};
