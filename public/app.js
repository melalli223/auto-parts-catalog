...
const promoSection=`<section class="tyreAdminCard tyreAdminPromoSection" id="tyrePromoSection"><div class="tyreCardHead"><div><span class="eyebrow">03 · PROMOTIONAL PANELS</span><h3>Winter, Summer & Custom Wheels</h3><p>Each panel has its own text, image and enquiry message.</p></div></div>
 ...
 const brandsSection=`<section class="tyreAdminCard" id="tyreBrandsSection"><div class="tyreCardHead"><div><span class="eyebrow">04 · BRANDS</span><h3>Tyre Brands</h3><p>Manage each tyre brand logo and its own page header image. The header image is separate for every brand.</p></div><button type="button" class="ghost" onclick="addTyreBrand()">+ ADD BRAND</button></div>
    <div class="tyreAdminBrandGrid">${brands.map((x,i)=>`<article class="tyreAdminBrandItem"><div class="tyreBrandNumber">BRAND ${i+1}<button type="button" class="tyreRemoveItemBtn" onclick="removeTyreBrand(${i})">Remove</button></div><div class="tyreAdminSubSection tyreBrandEditSubSection">${preview(x.image,(x.name||'Brand')+' logo')}${tyreInputRow('Brand name',`tb${i}n`,x.name)}<div class="formGroup"><label>Logo</label><input id="tb${i}i" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'tb${i}i')"></div><div class="tyreAdminSubHead tyreBrandHeaderUploadHead"><span>Brand page head image</span><small>This image belongs only to ${esc(x.name||'this brand')} and is used at the top of its tyre results page.</small></div>${preview(x.headerImage,(x.name||'Brand')+' page header')}<div class="formGroup"><label>Header image</label><input id="tb${i}h" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'tb${i}h')"></div></div></div></article>`).join('')}</div>
   </section>`;
 ...
 const featuredSection=`<section class="tyreAdminCard" id="tyreFeaturedSection"><div class="tyreCardHead"><div><span class="eyebrow">05 · TYRE TYPES</span><h3>Featured Tyre Types</h3><p>Add, edit or remove the tyre-type cards displayed below the brands.</p></div><button type="button" class="ghost" onclick="addTyreFeaturedType()">+ ADD TYPE</button></div>
    <div class="tyreAdminSubSection tyreTypesHeaderBackgroundSection"><div class="tyreAdminSubHead"><span>Featured tyre types header background</span><small>One background image is used for the header text area on all Featured Tyre Types pages.</small></div><div class="tyreAdminHeroLayout"><div class="formGroup"><label>Header background image</label><input id="ttHeaderBg" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'ttHeaderBg')"><small class="helpText">Upload one image. It will be shared by every Featured Tyre Types page.</small></div><div>${preview(t.featuredHeaderBackground,'Featured tyre types header background')}</div></div></div>
 ...
 function applyTyreEnquiryCustomRange(){const s=document.querySelector('#tyreEnquiryStart')?.value,e=document.querySelector('#tyreEnquiryEnd')?.value;if(!s||!e)return toast('Select both start and end dates');if(s>e)return toast('Start date must be before end date');loadTyreDashboardEnquiries('custom',s,e).then(renderTyreDashboardEnquiries)}
if(isDashboard)loadTyreDashboardEnquiries('month').then(renderTyreDashboardEnquiries);
}
...
function openTyreProductModal(index=-1){const t=db.tyres||defaultTyres,p=index>=0?clone((t.tyreProducts||[])[index]):{brandId:'',typeId:'',sizeId:'',tubeType:'Tubeless',season:'All Season',construction:'Radial',loadIndex:'',speedRating:'',availability:'Available',price:'',image:'',description:''},brands=t.brands||[],types=t.featured||[],sizes=t.sizes||[],selectedSizeIds=index>=0?[String(p.sizeId||'')]:[],sizeChoices=sizes.map((x,i)=>{const id=String(x.id||i),selected=selectedSizeIds.includes(id);return '<label class="tyreProductSizeChoice"><input type="checkbox" name="tpSizes" value="'+esc(id)+'" '+(selected?'checked':'')+' onchange="previewTyreProductName()"><span>'+esc(x.label)+'</span></label>'}).join(''),html='<div class="tyreSizeModalHead"><span class="eyebrow">TYRE PRODUCT</span><h2>'+(index>=0?'Edit tyre product':'Add tyre product')+'</h2><p class="muted">'+(index>=0?'Edit this tyre product.':'Select one or more tyre sizes to create multiple products at once.')+'</p></div><div class="tyreProductFormGrid"><div class="formGroup"><label>Tyre Brand</label><select id="tpBrand" class="select" onchange="previewTyreProductName()"><option value="">Select brand</option>'+brands.map((x,i)=>'<option value="'+esc(x.id||String(i))+'" '+((String(p.brandId)===String(x.id||i)||p.brand===x.name)?'selected':'')+'>'+esc(x.name)+'</option>').join('')+'</select></div><div class="formGroup"><label>Tyre Type</label><select id="tpType" class="select" onchange="previewTyreProductName()"><option value="">Select type</option>'+types.map((x,i)=>'<option value="'+esc(x.id||String(i))+'" '+((String(p.typeId)===String(x.id||i)||p.type===x.title)?'selected':'')+'>'+esc(x.title)+'</option>').join('')+'</select></div><div class="formGroup" style="grid-column:1/-1"><label>Tyre Sizes <span class="optional">'+(index>=0?'':'select multiple')+'</span></label><div id="tpSizes" class="tyreProductSizeChoices">'+sizeChoices+'</div></div><div class="formGroup"><label>Tube / Tubeless</label><select id="tpTube" class="select" onchange="previewTyreProductName()"><option>Tubeless</option><option>Tube Type</option><option>Not Specified</option></select></div><div class="formGroup"><label>Season</label><select id="tpSeason" class="select" onchange="previewTyreProductName()"><option>Summer</option><option>Winter</option><option>All Season</option><option>Not Specified</option></select></div><div class="formGroup"><label>Construction</label><select id="tpConstruction" class="select"><option>Radial</option><option>Bias</option><option>Not Specified</option></select></div><div class="formGroup"><label>Load Index <span class="optional">optional</span></label><input id="tpLoad" class="input" value="'+esc(p.loadIndex||'')+'" placeholder="91"></div><div class="formGroup"><label>Speed Rating <span class="optional">optional</span></label><input id="tpSpeed" class="input" value="'+esc(p.speedRating||'')+'" placeholder="V"></div><div class="formGroup"><label>Availability</label><select id="tpAvailability" class="select"><option>Available</option><option>Out of Stock</option></select></div><div class="formGroup"><label>Price</label><input id="tpPrice" class="input" type="number" min="0" step="0.01" inputmode="decimal" value="'+esc(p.price??'')+'" placeholder="0.00"><small class="helpText">Enter the selling price for this tyre product.</small></div><div class="formGroup" style="grid-column:1/-1"><label>Main Product Image</label>'+(p.image?'<div class="tyreProductCurrentImage"><img src="'+esc(p.image)+'" alt="Current product image"></div>':'')+'<input id="tpImage" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,&quot;tpImage&quot;)"><small class="helpText">'+(index>=0?'Choose a new image to replace the current product image. Leave empty to keep it.':'Upload the image for this product. When adding multiple sizes, the uploaded image is copied to each new product.')+'</small></div><div class="formGroup" style="grid-column:1/-1"><label>Short Description <span class="optional">optional</span></label><textarea id="tpDescription" class="textarea" rows="3">'+esc(p.description||'')+'</textarea></div></div><div class="tyreSizeLivePreview"><span>PRODUCT NAME'+(index<0?'S':'')+'</span><div id="tpNamePreview">'+esc(tyreProductName(p)||'Select brand, type and size')+'</div></div><button type="button" class="primary tyreSizeSaveButton" onclick="saveTyreProduct('+index+')">'+(index<0?'SAVE TYRE PRODUCTS':'SAVE TYRE PRODUCT')+'</button>';modal(html);document.querySelector('#tpTube').value=p.tubeType||'Tubeless';document.querySelector('#tpSeason').value=p.season||'All Season';document.querySelector('#tpConstruction').value=p.construction||'Radial';document.querySelector('#tpAvailability').value=p.availability||'Available';previewTyreProductName()}
function previewTyreProductName(){const get=id=>document.querySelector(id)?.value||'',brandId=get('#tpBrand'),typeId=get('#tpType'),tube=get('#tpTube'),season=get('#tpSeason'),selected=[...document.querySelectorAll('input[name="tpSizes"]:checked')].map(x=>x.value),t=db.tyres||defaultTyres,find=(arr,id)=>{const a=arr||[],direct=a.find(x=>String(x.id)===String(id));if(direct)return direct;const n=Number(id);return Number.isInteger(n)&&n>=0?a[n]:null},brand=find(t.brands,brandId)?.name||'',type=find(t.featured,typeId)?.title||'',el=document.querySelector('#tpNamePreview');if(!el)return;if(!brand||!type||!selected.length){el.textContent='Select brand, type and size';return}el.innerHTML=selected.map(id=>{const size=find(t.sizes,id)?.label||'';return '<div>'+esc([brand,type,size,tube,season].filter(Boolean).join(' '))+'</div>'}).join('')}
async function saveTyreProduct(index=-1){try{const t=clone(db.tyres||defaultTyres);t.tyreProducts=Array.isArray(t.tyreProducts)?t.tyreProducts:[];const brandId=document.querySelector('#tpBrand')?.value||'',typeId=document.querySelector('#tpType')?.value||'',sizeIds=[...document.querySelectorAll('input[name="tpSizes"]:checked')].map(x=>x.value),tubeType=document.querySelector('#tpTube')?.value||'Not Specified',season=document.querySelector('#tpSeason')?.value||'Not Specified',construction=document.querySelector('#tpConstruction')?.value||'Not Specified',loadIndex=document.querySelector('#tpLoad')?.value.trim()||'',speedRating=document.querySelector('#tpSpeed')?.value.trim()||'',availability=document.querySelector('#tpAvailability')?.value||'Available',price=document.querySelector('#tpPrice')?.value.trim()||'',description=document.querySelector('#tpDescription')?.value.trim()||'';if(!brandId||!typeId||!sizeIds.length)return toast('Select brand, tyre type and at least one tyre size');const f=document.querySelector('#tpImage')?.files?.[0],image=index>=0?t.tyreProducts[index]?.image||'':'';let uploadedImage=image;if(f)uploadedImage=await uploadImage(f,'product-images','tyre-products');const makeProduct=sizeId=>({id:'tp-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,7),brandId,typeId,sizeId,tubeType,season,construction,loadIndex,speedRating,availability,price,description,image:uploadedImage});if(index>=0){t.tyreProducts[index]={...t.tyreProducts[index],...makeProduct(sizeIds[0]),id:t.tyreProducts[index]?.id||makeProduct(sizeIds[0]).id};await saveTyreData(t);closeModal();toast('Tyre product updated');adminPanel('tyres');return}const existing=new Set(t.tyreProducts.map(x=>[x.brandId,x.typeId,x.sizeId].join('|'))),newProducts=[];for(const sizeId of sizeIds){const key=[brandId,typeId,sizeId].join('|');if(existing.has(key))continue;const product=makeProduct(sizeId);t.tyreProducts.push(product);newProducts.push(product)}if(!newProducts.length)return toast('All selected tyre sizes already have products for this brand and tyre type');await saveTyreData(t);closeModal();toast(newProducts.length+' tyre product'+(newProducts.length===1?'':'s')+' added');adminPanel('tyres')}catch(e){console.error(e);toast(e.message||'Could not save tyre product')}}
...
window.__tyreCompareIds=(()=>{try{const x=JSON.parse(localStorage.getItem('tyreCompareIds')||'[]');return Array.isArray(x)?x.map(String):[]}catch(e){return[]}})();
function tyreCompareProducts(){return (db.tyres?.tyreProducts||[]).filter(p=>window.__tyreCompareIds.includes(String(p.id)))}
function tyreCompareValue(p,key){if(key==='Brand')return (db.tyres?.brands||[]).find(x=>String(x.id)===String(p.brandId))?.name||p.brand||'';if(key==='Tyre Type')return (db.tyres?.featured||[]).find(x=>String(x.id)===String(p.typeId))?.title||p.type||'';if(key==='Tyre Size')return (db.tyres?.sizes||[]).find(x=>String(x.id)===String(p.sizeId))?.label||p.size||'';if(key==='Price')return p.price!==''&&p.price!=null?p.price:'';const map={'Tube/Tubeless':'tubeType',Season:'season',Construction:'construction','Load Index':'loadIndex','Speed Rating':'speedRating',Availability:'availability'};return p[map[key]]||''}
function tyreComparePersist(){try{localStorage.setItem('tyreCompareIds',JSON.stringify(window.__tyreCompareIds))}catch(e){}}
function ensureTyreCompareUI(){let el=document.querySelector('#tyreGlobalCompare');if(!el){el=document.createElement('div');el.id='tyreGlobalCompare';document.body.appendChild(el)}renderTyreCompareTray()}
function tyreCompareClear(){window.__tyreCompareIds=[];tyreComparePersist();renderTyreCompareTray();document.querySelectorAll('[data-tyre-compare]').forEach(b=>{b.textContent='Compare';b.classList.remove('selected')})}
function tyreCompareToggle(id){const key=String(id),ids=window.__tyreCompareIds;if(ids.includes(key))window.__tyreCompareIds=ids.filter(x=>x!==key);else{if(ids.length>=3)return toast('You can compare up to 3 tyre products');window.__tyreCompareIds=[...ids,key]}tyreComparePersist();ensureTyreCompareUI();document.querySelectorAll('[data-tyre-compare]').forEach(b=>{const selected=window.__tyreCompareIds.includes(String(b.dataset.tyreCompare));b.textContent=selected?'✓ Compared':'Compare';b.classList.toggle('selected',selected)})}
function renderTyreCompareTray(){const host=document.querySelector('#tyreGlobalCompare');if(!host)return;const ps=tyreCompareProducts();host.innerHTML=ps.length?'<div class="tyreCompareFloating"><div class="tyreCompareFloatingHead"><strong>Compare Products <span>('+ps.length+'/3)</span></strong><button type="button" class="tyreCompareFloatingClose" aria-label="Close comparison tray" onclick="tyreCompareClear()">×</button></div><div class="tyreCompareFloatingItems">'+ps.map(p=>'<div class="tyreCompareFloatingItem"><img src="'+esc(p.image||'')+'" alt=""><span>'+esc(tyreProductName(p))+'</span><button type="button" aria-label="Remove '+esc(tyreProductName(p))+'" onclick="tyreCompareToggle(&quot;'+esc(p.id)+'&quot;)">×</button></div>').join('')+'</div><div class="tyreCompareFloatingActions"><button type="button" class="tyreCompareFloatingCompare" onclick="openTyreCompare()">Compare</button><button type="button" class="tyreCompareFloatingClear" onclick="tyreCompareClear()">Clear</button></div></div>':''}
function closeTyreCompare(){const el=document.querySelector('#tyreCompareFull');if(el)el.remove();document.body.classList.remove('tyreCompareOpen')}
function openTyreCompare(){const ps=tyreCompareProducts();if(ps.length<2)return toast('Select at least 2 tyre products to compare');const specs=['Brand','Tyre Type','Tyre Size','Tube/Tubeless','Season','Construction','Load Index','Speed Rating','Price','Availability'];const el=document.createElement('div');el.id='tyreCompareFull';el.className='tyreCompareFull';el.innerHTML='<div class="tyreCompareFullTop"><button type="button" class="tyreCompareBack" onclick="closeTyreCompare()">← Back</button><div><span class="eyebrow">TYRE COMPARISON</span><h1>Compare Tyre Products</h1></div><button type="button" class="tyreCompareDone" onclick="closeTyreCompare()">Done</button></div><div class="tyreCompareFullScroll"><div class="tyreCompareFullGrid" style="--compare-cols:'+ps.length+'"><div class="tyreCompareFullCell tyreCompareFullLabel">Specification</div>'+ps.map(p=>'<div class="tyreCompareFullCell tyreCompareFullProduct"><button type="button" class="tyreCompareRemove" onclick="tyreCompareToggle(&quot;'+esc(p.id)+'&quot;);closeTyreCompare()">×</button><img src="'+esc(p.image||'')+'" alt=""><strong>'+esc(tyreProductName(p))+'</strong></div>').join('')+specs.map(k=>'<div class="tyreCompareFullCell tyreCompareFullLabel">'+esc(k)+'</div>'+ps.map(p=>'<div class="tyreCompareFullCell">'+esc(String(tyreCompareValue(p,k)||'—'))+'</div>').join('')).join('')+'</div></div></div>';document.body.appendChild(el);document.body.classList.add('tyreCompareOpen')}
function tyreProductCard(p){const name=tyreProductName(p),availability=p.availability||'Available',type=(db.tyres?.featured||[]).find(x=>String(x.id)===String(p.typeId))?.title||p.type||'',size=(db.tyres?.sizes||[]).find(x=>String(x.id)===String(p.sizeId))?.label||p.size||'',brands=db.tyres?.brands||[],brand=brands.find((x,i)=>String(x.id)===String(p.brandId)||String(i)===String(p.brandId)||String(x.name||'').toLowerCase()===String(p.brand||'').toLowerCase())||{},price=p.price!==''&&p.price!=null?String(p.price):'',loadSpeed=[p.loadIndex,p.speedRating].filter(Boolean).join('')||'',img=p.image?'<img src="'+esc(p.image)+'" alt="'+esc(name)+'">':'<span>TYRE</span>',brandImg=brand.image?'<img src="'+esc(brand.image)+'" alt="'+esc(brand.name||'Tyre brand')+'">':'<span>'+esc(brand.name||'BRAND')+'</span>',selected=window.__tyreCompareIds.includes(String(p.id));return '<article class="tyreShowcaseProductCard"><div class="tyreShowcaseImage">'+img+'</div><div class="tyreShowcaseBody"><div class="tyreShowcaseBrand">'+brandImg+'</div><h3>'+esc(name)+'</h3><div class="tyreShowcaseSpecs"><span>'+esc(size)+'</span><span>'+esc(type)+'</span><span>'+esc(p.season||'')+'</span><span>'+esc(p.tubeType||'')+'</span>'+(loadSpeed?'<span>'+esc(loadSpeed)+'</span>':'')+'</div><div class="tyreShowcasePrice"><small>Price</small><strong>'+esc(price||'—')+'</strong></div><div class="tyreShowcaseAvailability '+(String(availability).toLowerCase().includes('out')?'out':'')+'">'+esc(availability)+'</div><div class="tyreShowcaseActions"><button type="button" class="tyreShowcaseDetail" onclick="event.stopPropagation();tyreProductDetails(&quot;'+esc(p.id)+'&quot;)">View Detail</button><button type="button" class="tyreShowcaseCompare'+(selected?' selected':'')+'" data-tyre-compare="'+esc(p.id)+'" onclick="event.stopPropagation();tyreCompareToggle(&quot;'+esc(p.id)+'&quot;)">'+(selected?'✓ Compared':'Compare')+'</button></div></div></article>'}
function tyreProductBrand(p){return (db.tyres?.brands||[]).find(x=>String(x.id)===String(p.brandId)||String(x.name||'').toLowerCase()===String(p.brand||'').toLowerCase())||{}}
async function recordTyreEnquiry(p,quantity=1){
 try{
  if(!supabaseClient||!p)return;
  const t=db.tyres||defaultTyres,brands=t.brands||[],types=t.featured||[],sizes=t.sizes||[];
  const brand=brands.find((x,n)=>String(x.id)===String(p.brandId)||String(n)===String(p.brandId)||String(x.name||'').toLowerCase()===String(p.brand||'').toLowerCase());
  const type=types.find((x,n)=>String(x.id)===String(p.typeId)||String(n)===String(p.typeId));
  const size=sizes.find((x,n)=>String(x.id)===String(p.sizeId)||String(n)===String(p.sizeId));
  const name=tyreProductName(p)||'Tyre product';
  const {error}=await supabaseClient.from('product_enquiries').insert({product_id:null,product_name:name,brand_name:brand?.name||p.brand||'',model_name:size?.label||p.size||'',category_name:type?.title||p.type||'',year:null,admin_note:quantity>1?'Tyre quantity requested: '+quantity:''});
  if(error)console.warn('Tyre enquiry tracking failed:',error.message||error);
 }catch(e){console.warn('Tyre enquiry tracking failed:',e)}
}
function tyreProductDetails(id){const p=(db.tyres?.tyreProducts||[]).find(x=>String(x.id)===String(id));if(!p)return;const name=tyreProductName(p),brand=tyreProductName({...p,typeId:'',sizeId:''}).trim().replace(/\s+$/,''),type=((db.tyres?.featured||[]).find(x=>String(x.id)===String(p.typeId))?.title||((Number.isInteger(Number(p.typeId))?db.tyres?.featured?.[Number(p.typeId)]?.title:'')||p.type||'')),size=((db.tyres?.sizes||[]).find(x=>String(x.id)===String(p.sizeId))?.label||((Number.isInteger(Number(p.sizeId))?db.tyres?.sizes?.[Number(p.sizeId)]?.label:'')||p.size||''));modal('<div class="tyreProductDetail"><div class="tyreProductDetailImage">'+(p.image?'<img src="'+esc(p.image)+'" alt="'+esc(name)+'">':'<span>TYRE</span>')+'</div><div><span class="eyebrow">TYRE PRODUCT</span><h2>'+esc(name)+'</h2><div class="tyreDetailFacts">'+[['Brand',brand],['Tyre Type',type],['Tyre Size',size],['Tube/Tubeless',p.tubeType],['Season',p.season],['Construction',p.construction],['Load Index',p.loadIndex],['Speed Rating',p.speedRating],['Availability',p.availability],['Price',p.price]].filter(x=>x[1]!==''&&x[1]!=null).map(x=>'<div><small>'+esc(x[0])+'</small><strong>'+esc(x[1])+'</strong></div>').join('')+'</div>'+(p.description?'<p class="tyreProductDescription">'+esc(p.description)+'</p>':'')+'<div class="tyreEnquiryQty"><label>Quantity</label><div class="tyreQtyControl"><button type="button" onclick="this.nextElementSibling.stepDown()">−</button><input id="tyreEnquiryQty_'+esc(p.id)+'" type="number" min="1" step="1" value="1"><button type="button" onclick="this.previousElementSibling.stepUp()">+</button></div></div><button class="tyreShowcaseDetail tyreDetailEnquiry" onclick=\'(function(){const q=Math.max(1,parseInt(document.getElementById("tyreEnquiryQty_'+esc(p.id)+'").value,10)||1);recordTyreEnquiry('+JSON.stringify(p).replace(/'/g,'&#39;')+',q);smartEnquiry('+JSON.stringify('Hello, I would like to enquire about '+name+'.').replace(/'/g,'&#39;')+', '+JSON.stringify(p.image||'').replace(/'/g,'&#39;')+', '+JSON.stringify(name).replace(/'/g,'&#39;')+' )})()\'>ENQUIRE</button></div></div>')}
function tyreProductSliderScroll(direction){const rail=document.querySelector(".tyrePublicProductGrid");if(!rail)return;const card=rail.querySelector(".tyreShowcaseProductCard");const amount=card?card.getBoundingClientRect().width+parseFloat(getComputedStyle(rail).gap||"0"):rail.clientWidth*.8;rail.scrollBy({left:direction*amount,behavior:"smooth"})}
function showTyreProductResults(title,filter,brandHeader=null){const ps=(db.tyres?.tyreProducts||[]).filter(p=>filter(p));setNav('');const typeBg=esc(db.tyres?.featuredHeaderBackground||'');const header=brandHeader?(brandHeader.kind==='type'?'<div class="tyreTypeResultsHeader" style="background-image:url(&quot;'+typeBg+'&quot;)"><div class="tyreTypeHeaderCopy"><span class="eyebrow">FEATURED TYRE TYPE</span><h1 class="tyreTypeResultsTitle">'+esc(String(brandHeader.name||brandHeader.title||'').toUpperCase())+' TYRES</h1><p>'+esc(brandHeader.description||('Browse available '+String(brandHeader.name||'').toUpperCase()+' tyres.'))+'</p></div></div>':brandHeader.kind==='size'?'<div class="tyreBrandResultsHeader tyreSizeActionHeader"><div class="tyreSizeActionImageFrame"><h1><span class="tyreSizeActionNumber">'+esc(String(brandHeader.size||'')).replace(/([\/R])/g,'<span class="tyreSizeActionAccent">$1</span>')+'</span></h1></div><div class="tyreBrandHeaderCopy"><span class="eyebrow">TYRE SIZE</span><p>Tyres available in size <strong>'+esc(String(brandHeader.size||''))+'</strong>.</p></div></div>':brandHeader.kind==='car'?'<div class="tyreBrandResultsHeader tyreCarActionHeader"><div class="tyreCarActionImages"><div class="tyreBrandHeaderImageFrame"><img src="'+esc(brandHeader.image||'')+'" alt="'+esc(brandHeader.modelName||'')+'"></div></div><div class="tyreBrandHeaderCopy"><span class="eyebrow">TYRE VEHICLE</span><h1>'+esc(String(brandHeader.title||'').toUpperCase())+'</h1><p>Tyres for your '+esc(brandHeader.vehicleLabel||'')+'.</p></div></div>':'<div class="tyreBrandResultsHeader"><div class="tyreBrandHeaderImageFrame"><img src="'+esc(brandHeader.headerImage||brandHeader.image||'')+'" alt="'+esc(brandHeader.name||'')+' tyre brand"></div><div class="tyreBrandHeaderCopy"><span class="eyebrow">TYRE BRAND</span><h1>'+esc(String(brandHeader.name||'').toUpperCase())+' <sup class="tyreBrandRegistered" aria-label="registered trademark">®</sup></h1><p>'+esc(brandHeader.description||'')+'</p></div></div>'):'<button class="tyrePlaceholderBack" onclick="tyres()">← Back to Tyres</button>';render('<section class="tyreExactPage"><div class="tyreDesktop"><div class="tyreProductResultsPage">'+header+(brandHeader?'':'<div class="tyreProductResultsHead"><span>TYRE CATALOGUE</span><h1>'+esc(title)+'</h1><p>'+ps.length+' tyre product'+(ps.length===1?'':'s')+' found.</p></div>')+'<div class="tyreProductSlider"><button class="tyreProductSliderArrow tyreProductSliderPrev" type="button" aria-label="Previous products" onclick="tyreProductSliderScroll(-1)">‹</button><div class="tyrePublicProductGrid">'+(ps.map(tyreProductCard).join('')||'<div class="empty">No tyre products found for this selection.</div>')+'</div><button class="tyreProductSliderArrow tyreProductSliderNext" type="button" aria-label="Next products" onclick="tyreProductSliderScroll(1)">›</button></div><button class="tyrePlaceholderBack" onclick="location.hash=&quot;'+(brandHeader&&brandHeader.kind==='car'?'tyres/by-car':'tyres/by-size')+'&quot;">'+(brandHeader&&brandHeader.kind==='car'?'← Back to Find Car Tyre':'← Back to Find Tyre Size')+'</button></div></div></section>');ensureTyreCompareUI();}
...
async function addTyreBrand(){try{const t=clone(db.tyres||defaultTyres);t.brands.push({name:'New Brand',image:'',headerImage:''});await saveTyreData(t);toast('Brand added — fill in the details and Save');adminPanel('tyres')}catch(e){console.error(e);toast(e.message||'Could not add brand')}}
...
async function loadTyreDashboardEnquiries(range='month',customStart='',customEnd=''){
 const empty={total:0,today:0,week:0,month:0,products:[],brands:[],types:[],sizes:[],daily:[],statuses:{},recent:[],error:null,range};
 try{
  const t=db.tyres||defaultTyres,tyreBrands=(t.brands||[]).map(x=>String(x.name||'').trim().toLowerCase()).filter(Boolean),tyreTypes=(t.featured||[]).map(x=>String(x.title||'').trim().toLowerCase()).filter(Boolean),tyreSizes=(t.sizes||[]).map(x=>String(x.label||'').trim().toLowerCase()).filter(Boolean);
  const {data,error}=await supabaseClient.from('product_enquiries').select('*').order('created_at',{ascending:false}).limit(1000);
  if(error)throw error;
  const all=data||[];
  const rows=all.filter(x=>{
   const brand=String(x.brand_name||'').trim().toLowerCase(),type=String(x.category_name||'').trim().toLowerCase(),size=String(x.model_name||'').trim().toLowerCase(),name=String(x.product_name||'').toLowerCase();
   return tyreBrands.includes(brand)||tyreTypes.includes(type)||tyreSizes.includes(size)||/\btyres?\b|\btires?\b/.test(name);
  });
  const now=new Date(),day=new Date(now.getFullYear(),now.getMonth(),now.getDate()),week=new Date(day);week.setDate(week.getDate()-week.getDay());const month=new Date(now.getFullYear(),now.getMonth(),1);
  let start=month,end=new Date(now);if(range==='today')start=day;else if(range==='week')start=week;else if(range==='custom'&&customStart){start=new Date(customStart+'T00:00:00');end=new Date((customEnd||customStart)+'T23:59:59.999')}
  const inRange=(v,d)=>{const x=new Date(v);return !Number.isNaN(x.getTime())&&x>=d};const selected=rows.filter(x=>{const d=new Date(x.created_at);return !Number.isNaN(d.getTime())&&d>=start&&d<=end});
  const counts=key=>{const m=new Map();selected.forEach(x=>{const k=String(x[key]||'Unknown').trim()||'Unknown';m.set(k,(m.get(k)||0)+1)});return [...m.entries()].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name)).slice(0,10)};
  const daily=[];const dayCount=Math.min(31,Math.max(1,Math.ceil((end-start)/86400000)+1));for(let i=dayCount-1;i>=0;i--){const d=new Date(end);d.setHours(0,0,0,0);d.setDate(d.getDate()-i);const key=d.toISOString().slice(0,10);daily.push({date:key,count:selected.filter(x=>String(x.created_at||'').slice(0,10)===key).length})}
  const statusNames=['New','Contacted','Quoted','Sold','Closed'],statusCounts=Object.fromEntries(statusNames.map(x=>[x,0]));selected.forEach(x=>{const s=statusNames.includes(x.status||'New')?(x.status||'New'):'New';statusCounts[s]++});
  return {total:selected.length,today:rows.filter(x=>inRange(x.created_at,day)).length,week:rows.filter(x=>inRange(x.created_at,week)).length,month:rows.filter(x=>inRange(x.created_at,month)).length,products:counts('product_name'),brands:counts('brand_name'),types:counts('category_name'),sizes:counts('model_name'),daily,statuses:statusCounts,recent:selected.slice(0,6),error:null,range,start:start.toISOString().slice(0,10),end:end.toISOString().slice(0,10)};
 }catch(e){console.warn('Could not load tyre enquiry analytics:',e);return {...empty,error:e}}
}
function renderTyreDashboardEnquiries(e){
 const box=document.querySelector('#tyreEnquiryStats');if(!box)return; const filters='<div class="analyticsFilters"><div class="filterPills"><button type="button" class="analyticsFilter '+(e.range==='today'?'active':'')+'" onclick="loadTyreDashboardEnquiries(\'today\').then(renderTyreDashboardEnquiries)">Today</button><button type="button" class="analyticsFilter '+(e.range==='week'?'active':'')+'" onclick="loadTyreDashboardEnquiries(\'week\').then(renderTyreDashboardEnquiries)">This Week</button><button type="button" class="analyticsFilter '+(e.range==='month'?'active':'')+'" onclick="loadTyreDashboardEnquiries(\'month\').then(renderTyreDashboardEnquiries)">This Month</button><button type="button" class="analyticsFilter '+(e.range==='custom'?'active':'')+'" onclick="applyTyreEnquiryCustomRange()">Custom Range</button></div><div class="customDateFields"><input id="tyreEnquiryStart" type="date" value="'+esc(e.range==='custom'?e.start:'')+'"><span>to</span><input id="tyreEnquiryEnd" type="date" value="'+esc(e.range==='custom'?e.end:'')+'"><button type="button" class="ghost" onclick="applyTyreEnquiryCustomRange()">APPLY</button></div></div>';
 const statusNames=['New','Contacted','Quoted','Sold','Closed'],statusCounts=e.statuses||Object.fromEntries(statusNames.map(x=>[x,0])),maxDaily=Math.max(1,...e.daily.map(x=>Number(x.count)||0)),maxProduct=Math.max(1,...e.products.map(x=>x.count||0)),maxBrand=Math.max(1,...e.brands.map(x=>x.count||0)),maxType=Math.max(1,...e.types.map(x=>x.count||0)),maxSize=Math.max(1,...e.sizes.map(x=>x.count||0)),totalStatus=Math.max(1,statusNames.reduce((a,k)=>a+(statusCounts[k]||0),0));
 const dayLabels=e.daily.map(x=>{const d=new Date(String(x.date||'')+'T00:00:00');return Number.isNaN(d.getTime())?String(x.date||''):d.toLocaleDateString([], {month:'short',day:'numeric'})});
 const rank=(items,max)=>items.length?items.slice(0,6).map((x,i)=>'<div class="rankRow"><span class="rank">'+(i+1)+'</span><div class="rankInfo"><strong>'+esc(x.name||'Unknown')+'</strong><div class="bar"><i style="width:'+Math.round((Number(x.count)||0)/max*100)+'%"></i></div></div><b>'+Number(x.count||0)+'</b></div>').join(''):'<p class="muted">No tyre enquiry data yet.</p>';
 const simple=(items,max)=>items.length?items.slice(0,6).map(x=>'<div class="simpleRank"><span>'+esc(x.name||'Unknown')+'</span><b>'+Number(x.count||0)+'</b><div class="bar"><i style="width:'+Math.round((Number(x.count)||0)/max*100)+'%"></i></div></div>').join(''):'<p class="muted">No tyre enquiry data yet.</p>';
 box.innerHTML=filters+'<div class="dashboardSectionTitle"><span class="eyebrow">CUSTOMER DEMAND</span><h3>Enquiry intelligence</h3><p class="muted">Live customer enquiry activity for the tyre catalogue.</p></div><div class="grid stats"><div class="card stat"><b>'+e.total+'</b><span>Total Enquiries</span></div><div class="card stat"><b>'+e.today+'</b><span>Today</span></div><div class="card stat"><b>'+e.week+'</b><span>This Week</span></div><div class="card stat"><b>'+e.month+'</b><span>This Month</span></div></div><div class="analyticsOverview"><div class="card analyticsCard analyticsChartCard"><div class="analyticsCardHead"><div><span class="eyebrow">LAST 14 DAYS</span><h3>Enquiry activity</h3></div><span class="analyticsHint">Tyre requests per day</span></div>'+(e.daily.length?'<div class="dailyChart" role="img" aria-label="Daily tyre enquiries for the last 14 days">'+e.daily.map((x,i)=>'<div class="dailyBarCol"><span class="dailyValue">'+Number(x.count||0)+'</span><div class="dailyBarTrack"><i style="height:'+Math.max(5,Math.round((Number(x.count)||0)/maxDaily*100))+'%"></i></div><small>'+esc(dayLabels[i]||'')+'</small></div>').join('')+'</div>':'<p class="muted">No daily enquiry data yet.</p>')+'</div><div class="card analyticsCard statusCard"><div class="analyticsCardHead"><div><span class="eyebrow">PIPELINE</span><h3>Enquiry status</h3></div><button class="ghost" onclick="adminPanel(\'enquiries\')">OPEN CRM</button></div>'+statusNames.map(name=>'<div class="statusMetric"><div><span>'+name+'</span><b>'+Number(statusCounts[name]||0)+'</b></div><div class="statusBar"><i style="width:'+Math.round((statusCounts[name]||0)/totalStatus*100)+'%"></i></div></div>').join('')+'</div></div><div class="analyticsGrid"><div class="card analyticsCard"><span class="eyebrow">TOP PRODUCTS</span><h3>Most Enquired Products</h3>'+rank(e.products,maxProduct)+'</div><div class="card analyticsCard"><span class="eyebrow">BRANDS</span><h3>Most Enquired Brands</h3>'+simple(e.brands,maxBrand)+'</div><div class="card analyticsCard"><span class="eyebrow">TYRE TYPES</span><h3>Most Enquired Types</h3>'+simple(e.types,maxType)+'</div><div class="card analyticsCard"><span class="eyebrow">TYRE SIZES</span><h3>Most Enquired Sizes</h3>'+simple(e.sizes,maxSize)+'</div></div>'+(e.error?'<div class="adminTip"><strong>Tyre enquiry data could not be loaded.</strong><span>Check the product_enquiries table and admin read policy.</span></div>':'<div class="recentEnquiryPanel card"><div class="dashboardPanelHead"><div><span class="eyebrow">RECENT CUSTOMER ACTIVITY</span><h3>Latest Tyre Enquiries</h3></div><button class="ghost" onclick="adminPanel(\'enquiries\')">OPEN CRM</button></div>'+(e.recent.length?'<div class="recentEnquiryList">'+e.recent.map(x=>'<div class="recentEnquiry"><div><strong>'+esc(x.product_name||'Tyre enquiry')+'</strong><small>'+esc([x.brand_name,x.category_name,x.model_name].filter(Boolean).join(' · ')||'Tyre details not provided')+'</small></div><span class="enquiryStatus '+enquiryStatusClass(x.status)+'">'+esc(x.status||'New')+'</span><time>'+enquiryTime(x.created_at)+'</time></div>').join('')+'</div>':'<p class="muted">No tyre customer enquiries recorded yet.</p>')+'</div>')+'';
}
...
function modelAdmin(c){c.innerHTML=`<div class="adminHead"><div><h2>Manage models</h2><p class="muted">Add models and assign the tyre type that applies to every year of the model.</p></div><button class="primary" onclick="modelForm()">+ ADD MODEL</button></div><table class="table"><tr><th>Model</th><th>Brand</th><th>Tyre Type</th><th>Years</th><th>Actions</th></tr>${db.models.map(m=>{const b=db.brands.find(x=>x.id===m.brandId);const tt=(db.tyres?.featured||[]).find(x=>String(x.id)===String(m.tyreTypeId));return `<tr><td><div class="tableBrand"><img class="thumb" src="${m.image||placeholder(m.name)}">${esc(m.name)}</div></td><td>${esc(b?.name)}</td><td>${esc(tt?.title||'Not assigned')}</td><td>${modelYears(m.id).join(', ')||'—'}</td><td><button class="ghost" onclick="modelEditForm('${m.id}')">EDIT</button> <button class="danger" onclick="delModel('${m.id}')">Delete</button></td></tr>`}).join('')}</table>`}
...
function tyreTypeOptions(selected=''){const types=(db.tyres?.featured||[]).filter(Boolean);return '<option value="">Not assigned</option>'+types.map(x=>`<option value="${esc(x.id||'')}" ${String(x.id)===String(selected)?'selected':''}>${esc(x.title||'Untitled type')}</option>`).join('')}
function modelForm(){modal(`<h2>Add model</h2><div class="formGroup"><label>Brand</label><select id="m1" class="select" onchange="updateModelTypeField()">${db.brands.map(b=>`<option value="${b.id}">${esc(b.name)}</option>`).join('')}</select></div><div id="modelTypeWrap" class="formGroup" style="display:none"><label>Model type</label><select id="mType" class="select"><option value="regular">Car</option><option value="ev">EV</option></select></div><div class="formGroup"><label>Tyre Type</label><select id="mTyreType" class="select">${tyreTypeOptions('')}</select><small class="helpText">Every year under this model inherits this classification.</small></div><div class="formGroup"><label>Model</label><input id="m2" class="input" placeholder="e.g. Corolla"></div><div class="formGroup"><label>Model image</label><input id="m3" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'m3')"><small class="helpText">This image is shown at the top after the customer chooses the model.</small></div><button class="primary" onclick="addModel()">SAVE</button>`);updateModelTypeField()}
...
async function addModel(){const name=document.querySelector('#m2')?.value.trim(),brandId=document.querySelector('#m1')?.value,brand=db.brands.find(b=>b.id===brandId),type=document.querySelector('#mType')?.value,tyreTypeId=document.querySelector('#mTyreType')?.value||null;const isEv=brand?.isEv===true&&brand?.isRegular===false?true:brand?.isEv===true&&brand?.isRegular!==false?(type==='ev'):false;if(!name||!brandId)return toast('Enter a model and select a brand');try{const f=editedImage('m3');const image=f?await uploadImage(f,'model-images','models'):'';const {data,error}=await supabaseClient.from('models').insert({brand_id:brandId,name,image_url:image||null,is_ev:isEv,tyre_type_id:tyreTypeId,sort_order:0,active:true}).select().single();if(error)throw error;db.models.push({id:data.id,brandId,name,image:data.image_url||'',isEv,tyreTypeId:data.tyre_type_id||''});cacheDb();closeModal();adminPanel('models');toast('Model saved')}catch(e){console.error(e);toast(e.message||'Could not save model')}}
function modelEditForm(id){const m=db.models.find(x=>x.id===id);if(!m)return;modal(`<h2>Edit model</h2><div class="formGroup"><label>Brand</label><select id="em1" class="select">${db.brands.map(b=>`<option value="${b.id}" ${b.id===m.brandId?'selected':''}>${esc(b.name)}</option>`).join('')}</select></div><div class="formGroup"><label>Tyre Type</label><select id="emTyreType" class="select">${tyreTypeOptions(m.tyreTypeId||'')}</select><small class="helpText">All years belonging to this model inherit this classification.</small></div><div class="formGroup"><label>Model</label><input id="em2" class="input" value="${esc(m.name)}"></div><div class="formGroup"><label>Replace model image <span class="optional">(optional)</span></label><input id="em3" type="file" accept="image/*" class="input" onchange="prepareImageSelection(event,'em3')"></div><button class="primary" onclick="saveModelEdit('${id}')">SAVE CHANGES</button>`)}
async function saveModelEdit(id){const m=db.models.find(x=>x.id===id);if(!m)return;const name=document.querySelector('#em2')?.value.trim(),brandId=document.querySelector('#em1')?.value,tyreTypeId=document.querySelector('#emTyreType')?.value||null;if(!name||!brandId)return toast('Enter a model and select a brand');try{const f=editedImage('em3');let image=m.image||'';if(f)image=await uploadImage(f,'model-images','models');const {data,error}=await supabaseClient.from('models').update({brand_id:brandId,name,image_url:image||null,tyre_type_id:tyreTypeId}).eq('id',id).select().single();if(error)throw error;m.brandId=data.brand_id;m.name=data.name;m.image=data.image_url||'';m.tyreTypeId=data.tyre_type_id||'';m.isEv=data.is_ev===true;cacheDb();closeModal();adminPanel('models');toast('Model updated')}catch(e){console.error(e);toast(e.message||'Could not update model')}}
...
function modal(html){const d=document.createElement('div');d.className='modal';d.id='modal';const tyreDetail=String(html).includes('tyreProductDetail');d.innerHTML=`<div class="modalBox">${html}<button class="${tyreDetail?'tyreDetailClose':'ghost'}" type="button" onclick="closeModal()">${tyreDetail?'×':'CANCEL'}</button></div>`;document.body.appendChild(d)}
...
async function delModel(id){if(!confirm('Delete model? Existing years and products are not deleted.'))return;try{const {error}=await supabaseClient.from('models').delete().eq('id',id);if(error)throw error;db.models=db.models.filter(m=>m.id!==id);db.years=db.years.filter(y=>y.modelId!==id);db.parts=db.parts.filter(p=>p.modelId!==id);cacheDb();adminPanel('models');toast('Model deleted')}catch(e){console.error(e);toast(e.message||'Could not delete model')}}
