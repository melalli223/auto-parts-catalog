...
function previewTyrePromoUploads(input){
 ...
 gallery.innerHTML=urls.length?'<div class="tyreAdminImageGallery">'+urls.map((src,i)=>'<div class="tyreAdminGalleryItem"><img src="'+src+'" alt="New promo image '+(i+1)+'"><span>NEW IMAGE '+(i+1)+'</span></div>').join('')+'</div>':'';
 const defaults=defaultTyres.promoTexts||[];
 editor.innerHTML=urls.map((src,i)=>{const d=defaults[i%defaults.length]||{};return '<div class="tyrePromoTextCard"><button type="button" class="tyrePromoImageButton" onclick="toggleTyrePromoText('+i+')"><img src="'+src+'" alt="New promo image '+(i+1)+'"><span>NEW IMAGE '+(i+1)+'</span><b>EDIT TEXT</b></button><div id="tyrePromoTextForm'+i+'" class="tyrePromoTextForm" hidden><div class="tyreAdminSubHead"><span>New image '+(i+1)+' text</span><small>Default text is ready to edit.</small></div><div class="tyrePromoTextFields"><div class="formGroup"><label>Heading</label><input id="tPromoTitle'+i+'" class="input" value="'+esc(d.title||'')+'"></div><div class="formGroup"><label>Highlight</label><input id="tPromoHighlight'+i+'" class="input" value="'+esc(d.highlight||'')+'"></div><div class="formGroup"><label>Description</label><textarea id="tPromoDesc'+i+'" class="input" rows="2">'+esc(d.desc||'')+'</textarea></div><div class="formGroup"><label>Button text</label><input id="tPromoButton'+i+'" class="input" value="'+esc(d.button||'')+'"></div></div></div></div>';}).join('');
}
