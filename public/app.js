...
function tyreTypePage(idx){const t=db.tyres||defaultTyres,x=(t.featured||defaultTyres.featured)[idx];if(!x)return tyres();location.hash=`tyres/type/${idx}`;showTyreProductResults(`${x.title} Tyres`,p=>String(p.typeId)===String(x.id)||String(p.typeId)===String(idx)||String(p.type||'').toLowerCase()===String(x.title).toLowerCase(),{...x,kind:"type"})}
