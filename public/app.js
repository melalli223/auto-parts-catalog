...
function tyreProductCard(p){const name=tyreProductName(p),availability=p.availability||'Available',type=(db.tyres?.featured||[]).find(x=>String(x.id)===String(p.typeId))?.title||p.type||'',size=(db.tyres?.sizes||[]).find(x=>String(x.id)===String(p.sizeId))?.label||p.size||'',brand=(db.tyres?.brands||[]).find(x=>String(x.id)===String(p.brandId))||{},price=p.price!==''&&p.price!=null?String(p.price):'',loadSpeed=[p.loadIndex,p.speedRating].filter(Boolean).join('')||'',img=p.image?'<img src="'+esc(p.image)+'" alt="'+esc(name)+'">':'<span>TYRE</span>',brandImg=brand.image?'<img src="'+esc(brand.image)+'" alt="'+esc(brand.name||'Tyre brand')+'">':'<span>'+esc(brand.name||'BRAND')+'</span>';return '<article class="tyreShowcaseProductCard"><div class="tyreShowcaseImage">'+img+'</div><div class="tyreShowcaseBody"><div class="tyreShowcaseBrand">'+brandImg+'</div><h3>'+esc(name)+'</h3><div class="tyreShowcaseSpecs"><span><b>Size</b>'+esc(size)+'</span><span><b>Type</b>'+esc(type)+'</span><span><b>Season</b>'+esc(p.season||'')+'</span><span><b>Tube</b>'+esc(p.tubeType||'')+'</span>'+(loadSpeed?'<span><b>Load/Speed</b>'+esc(loadSpeed)+'</span>':'')+'</div><div class="tyreShowcasePrice"><small>Price</small><strong>'+esc(price||'—')+'</strong></div><div class="tyreShowcaseAvailability '+(String(availability).toLowerCase().includes('out')?'out':'')+'">'+esc(availability)+'</div><div class="tyreShowcaseActions"><button type="button" class="tyreShowcaseDetail" onclick="event.stopPropagation();tyreProductDetails(&quot;'+esc(p.id)+'&quot;)">View Detail</button><button type="button" class="tyreShowcaseCompare" onclick="event.stopPropagation();toast(&quot;Compare is not available yet&quot;)">Compare</button></div></div></article>'}
